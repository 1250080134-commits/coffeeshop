...
<<<<<<< HEAD
  ...
  // Also capture grind_size / selected_weight per line item
  const itemMap     = new Map();   // product_id → { qty, grind_size, selected_weight }
  for (const item of items) {
    ...
    if (itemMap.has(pid)) {
      itemMap.get(pid).qty += qty;
    } else {
      itemMap.set(pid, {
        qty,
        grind_size:      item.grind_size      || null,
...
<<<<<<< HEAD
    for (const [productId, { qty: quantity, grind_size, selected_weight }] of itemMap.entries()) {
      ...
      detailRows.push({ product_id: productId, quantity, unit_price, grind_size, selected_weight });
    }
